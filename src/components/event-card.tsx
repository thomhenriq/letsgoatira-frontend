...
export function EventCard({ event }: { event: Event }) {
  return (
    <Link href={`/eventos/${event.id}`} className="group flex rounded-md overflow-hidden border border-zinc-200 bg-white">
      ...
      <div className="flex flex-col justify-between p-3 sm:p-4 gap-2 min-w-0">
        <div>
          <h2 className="font-semibold text-zinc-800 text-sm sm:text-base leading-tight group-hover:text-primary transition-colors truncate">
            {event.title}
          </h2>
          <p className="text-zinc-500 text-xs sm:text-sm mt-1 line-clamp-2">
            {event.description}
          </p>
        </div>
